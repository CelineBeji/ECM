...
class TwoClassifiers:
    ...
    def fit(self, data, predictors):
        ...
        data_t = data[data['treatment'] == 1]  # treatment group
        data_c = data[data['treatment'] == 0]  # control group

        # Fit on treatment group
        self.model_t.fit(data_t[predictors], data_t['outcome'])
        # Fit on control group
        self.model_c.fit(data_c[predictors], data_c['outcome'])
    ...
    def predict(self, data, predictors):
        ...
        # prediction of type
        data.loc[np.logical_and(np.logical_and(data['treatment'] == 1, data['outcome'] == 1), self.model_c.predict(
            data[predictors]) == 0), 'typePredict'] = 'responder'
        data.loc[np.logical_and(np.logical_and(data['treatment'] == 0, data['outcome'] == 0), self.model_t.predict(
            data[predictors]) == 1), 'typePredict'] = 'responder'

        data.loc[np.logical_and(np.logical_and(data['treatment'] == 1, data['outcome'] == 0), self.model_c.predict(
        ...
        data.loc[np.logical_and(np.logical_and(data['treatment'] == 0, data['outcome'] == 1), self.model_t.predict(
            data[predictors]) == 1), 'typePredict'] = 'survivor'
    ...
    def GridSearchCV(self, data, predictors, params):
        ...
        g1.fit(data[data['treatment'] == 1][predictors],
               data[data['treatment'] == 1]['outcome'])
        # Fit on control group
        g2 = GridSearchCV(self.model_c, params,
                          cv=KFold(n_splits=20, shuffle=True))
        g2.fit(data[data['treatment'] == 0][predictors],
