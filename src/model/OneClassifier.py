...
class OneClassifier:
    ...
    def fit(self, data, predictors):
        ...
        predictorTreatment = predictors + ['treatment']
        # Fit on treatment group
        self.model.fit(data[predictorTreatment], data['outcome'])
    ...
    def predict(self, data, predictors):
        ...
        data['treatmentPredict'] = 1 - data['treatment']
        ...
        # prediction of type
        data.loc[np.logical_and(np.logical_and(data['treatment'] == 1, data['outcome']
                                               == 1), data['outcomePredict'] == 0), 'typePredict'] = 'responder'
        data.loc[np.logical_and(np.logical_and(data['treatment'] == 0, data['outcome']
                                               == 0), data['outcomePredict'] == 1), 'typePredict'] = 'responder'
        data.loc[np.logical_and(
            data['outcome'] == 0, data['outcomePredict'] == 0), 'typePredict'] = 'doomed'
        data.loc[np.logical_and(
            data['outcome'] == 1, data['outcomePredict'] == 1), 'typePredict'] = 'survivor'
        data.loc[np.logical_and(np.logical_and(data['treatment'] == 1, data['outcome'] == 0),
                                data['outcomePredict'] == 1), 'typePredict'] = 'anti-responder'
        data.loc[np.logical_and(np.logical_and(data['treatment'] == 0, data['outcome'] == 1),
    ...
    def GridSearchCV(self, data, predictors, params):
        ...
        g.fit(data[predictorTreatment], data['outcome'])
